...
fn write_line<P: Precision + WordType<BITS>, const BITS: usize>(
    ...
    let approximation: EstimatedUnionCardinalities<f32> =
        hll1.estimate_union_and_sets_cardinality(&hll2);
...
fn test_union_cardinality_perfs() {
    ...
    (0..number_of_tests)
        ...
        .for_each(|i| {
            ...
            let mut writer =
                BufWriter::with_capacity(128 * 1024, GzEncoder::new(file, Compression::default()));

            writer
                .write_all(b"precision\tbits\texact\thll\tnn\n")
                .unwrap();
            ...
            write_line_set_for_hasher(&vec1, &vec2, set1.len(), set2.len(), exact, &mut writer);
