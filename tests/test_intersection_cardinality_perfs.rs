...
fn new_intersection_hll<PRECISION: Precision + WordType<BITS>, const BITS: usize>(
    set1: &HashSet<u64>,
    set2: &HashSet<u64>,
) -> f32 {
    let hll1: HyperLogLog<PRECISION, BITS> = set1.iter().collect();
...
fn write_line<PRECISION: Precision + WordType<BITS>, const BITS: usize>(
    ...
    let new_hll = new_intersection_hll::<PRECISION, BITS>(&set1, &set2);
...
fn test_intersection_cardinality_perfs() {
    ...
    file.write_all(b"precision\tbits\texact\told_approximation\tnew_approximation\n")
        .unwrap();
