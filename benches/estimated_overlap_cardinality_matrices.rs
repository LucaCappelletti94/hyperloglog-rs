...
fn populate_vectors<const N: usize>() -> ([HyperLogLog<4, 4>; N], [HyperLogLog<4, 4>; N]) {
    ...
    // Create the counters
    let mut left: [HyperLogLog<4, 4>; N] = [HyperLogLog::new(); N];
    let mut right: [HyperLogLog<4, 4>; N] = [HyperLogLog::new(); N];
...
#[bench]
fn bench_estimated_overlap_cardinality_matrices_2(b: &mut Bencher) {
    let (left, right) = populate_vectors::<2>();

    b.iter(|| {
        // Inner closure, the actual test
        black_box(HyperLogLog::estimated_overlap_cardinality_matrices(
            &left, &right,
        ));
    });
}
...
#[bench]
fn bench_estimated_overlap_cardinality_matrices_3(b: &mut Bencher) {
...
#[bench]
fn bench_estimated_overlap_cardinality_matrices_4(b: &mut Bencher) {
...
#[bench]
fn bench_estimated_overlap_cardinality_matrices_5(b: &mut Bencher) {
