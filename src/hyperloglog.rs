...
impl EstimatedUnionCardinalities {
    ...
    /// ```
    ///
    /// With a negative offset that is equal to the union size, the Jaccard index is 0:
    ///
    /// ```
    /// use hyperloglog_rs::EstimatedUnionCardinalities;
    ///
    /// let estimated_union_cardinalities = EstimatedUnionCardinalities::from((2.0, 2.0, 2.0));
    ///
    /// let jaccard_index = unsafe{estimated_union_cardinalities.get_jaccard_index_with_offset(-2.0)};
    /// let expected = 0.0;
    ///
    /// assert_eq!(jaccard_index, expected, "Example 3: Expected {}, got {}", expected, jaccard_index);
    ///
    /// ```
    ///
    /// We test more extensively the last case we have seen by involving alsp
    /// the HyperLogLog counters to see if the Jaccard index is correctly computed:
    ///
    /// ```
    ...
    /// hll2.insert(&6);
    ///
    /// let estimated_union_cardinalities = hll1.estimate_union_and_sets_cardinality(&hll2);
    ///
    /// let jaccard_index = estimated_union_cardinalities.get_jaccard_index();
    /// let expected = 2.0 / 6.0;
    ///
    /// assert!(jaccard_index > expected * 0.9 && jaccard_index < expected * 1.1, "Example 4a: Expected around {}, got {}", expected, jaccard_index);
    ///
    /// let offsetted_jaccard_index = unsafe{estimated_union_cardinalities.get_jaccard_index_with_offset(-6.0)};
    /// let expected = 0.0;
    ///
    /// assert_eq!(
    ///     offsetted_jaccard_index, expected,
    ///     "Example 4b: Expected {}, got {}, with struct {:?}",
    ///     expected, offsetted_jaccard_index,
    ...
    pub unsafe fn get_jaccard_index_with_offset(&self, offset: f32) -> f32 {
        let intersection_cardinality =
            self.left_cardinality + self.right_cardinality - self.union_cardinality;
        let offsetted_union_cardinality = self.union_cardinality + offset;

        if intersection_cardinality > offsetted_union_cardinality {
            return 0.0;
        }

        (intersection_cardinality / (offsetted_union_cardinality).max(f32::EPSILON))
            .max(0.0)
            .min(1.0)
    }
}
